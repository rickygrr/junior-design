...
var Grid = /** @class */ (function (_super) {
    ...
    Grid.prototype.render = function () {
        ...
        var colSpans = this._columns.map(function () { return 1; });
        var isColHidden = this._columns.map(function () { return false; });
        if (this.props.columnVirtualization) {
            var widths = this._columns.map(function (c) { return parseFloat((c.width || '').toString()) || c.minResizableWidth || 10; });
            ...
            for (var i = this._columns.length - 1; i >= 0; i--) {
                ...
                if (widthOfHiddenColumns + 2 * widths[i] < totalWidth - tableWidth - scrollLeft) {
                    isColHidden[i] = true;
                    countOfHidden++;
                    widthOfHiddenColumns += widths[i];
                }
                else {
                    colSpans[i] = Math.max(colSpans[i], 1 + countOfHidden);
                    break;
                }
            }
        }
